...
export function initGradientDescentVisualizer(containerId) {
  ...
  container.innerHTML = `
    <div style="font-family: system-ui, sans-serif;">
      <div style="display: flex; gap: 16px; align-items: flex-start; flex-wrap: wrap;">
        <canvas id="${containerId}-cv" width="680" height="680" style="border: 1px solid #ccc; border-radius: 8px; max-width: 100%; height: auto;"></canvas>

        <div style="display: flex; flex-direction: column; gap: 10px; max-width: 760px;">
          <div style="display: inline-flex; gap: 8px; align-items: center;">
            <label>
              γ (anisotropy)
              <input id="${containerId}-gamma" type="range" min="1" max="200" value="20" style="width: 200px;" />
              <span id="${containerId}-gammaVal">20</span>
            </label>
          </div>
          ...
          <div style="display: flex; gap: 8px;">
            ...
            <button id="${containerId}-step">Step</button>
            <button id="${containerId}-play">Play</button>
            <button id="${containerId}-pause">Pause</button>
          </div>

          <div style="background: #f6f6f6; padding: 8px; border-radius: 6px; font-size: 0.9em;">
          ...
          <div id="${containerId}-status" style="font-size: 0.9em;"></div>
        </div>
      </div>
    </div>
  `;
  ...
  const gammaSlider = document.getElementById(`${containerId}-gamma`);
  const gammaVal = document.getElementById(`${containerId}-gammaVal`);
  const ruleSel = document.getElementById(`${containerId}-rule`);
  const status = document.getElementById(`${containerId}-status`);

  const btnReset = document.getElementById(`${containerId}-reset`);
  const btnStep  = document.getElementById(`${containerId}-step`);
  const btnPlay  = document.getElementById(`${containerId}-play`);
  const btnPause = document.getElementById(`${containerId}-pause`);
  ...
  function tick() {
    if (playing) stepOnce();
    // Slow down the animation loop for visibility
    animationId = setTimeout(() => requestAnimationFrame(tick), 200);
  }

  btnReset.onclick = reset;
  btnStep.onclick  = stepOnce;
  btnPlay.onclick  = () => { if(!playing) { playing = true; tick(); } };
  btnPause.onclick = () => { playing = false; clearTimeout(animationId); };
